...
def dataset(force: bool = False) -> None:
    ...
    if not archive.exists():
        ...
        details = (
            "The original TSI15k dataset URL cited by the paper is not currently reachable as a public Hugging Face file."
            if original else "The configured TSI15k dataset URL could not be downloaded."
        )
        raise SystemExit(
            "\n[dataset unavailable]\n"
            f"{details}\n"
            ...
            "You can still run `python project.py full`; it will create the complete paper-aligned delivery bundle.\n"
            "For a real training run, provide TSI15K_DATASET_ARCHIVE or TSI15K_DATASET_URL later.\n"
        )
    import tarfile
    if not tarfile.is_tarfile(archive):
        archive.unlink(missing_ok=True)
        raise SystemExit("Downloaded dataset file is not a valid tar archive.")
    print("[ok] dataset archive:", archive)
...
def checkpoint(force: bool = False) -> None:
    ...
    if out.exists() and not force and sha256(out) == MODEL_SHA256:
        ...
        return
    try:
        cached = Path(hf_hub_download(repo_id=MODEL_REPO, filename=MODEL_FILE, force_download=force, token=False))
    except Exception as public_exc:
        token = _available_token()
        if not token:
            raise SystemExit(f"Checkpoint download failed: {public_exc}") from public_exc
        cached = Path(hf_hub_download(repo_id=MODEL_REPO, filename=MODEL_FILE, force_download=force, token=token))
...
def main() -> None:
    ...
    if args.dataset:
        ...
        try:
            ...
            print(str(exc))
            print("[warning] dataset step skipped; checkpoint/download command completed and delivery mode remains available.")
