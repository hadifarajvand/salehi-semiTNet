...
def main():
    ...
    if not (vendor/'PROJECT_PATCH_MANIFEST.json').exists(): raise SystemExit('Run `make bootstrap` first.')
    ...
    if not ckpt.exists(): raise SystemExit('Model checkpoint missing; run `make download`.')
    env=os.environ.copy(); env['TSI15K_ROOT']=str(dataset); env['NUM_GPUS']=str(a.num_gpus); env['CHECKPOINT']=str(ckpt)
    subprocess.run(['bash','project_eval_released.sh'],cwd=vendor,env=env,check=True)
    ...
    import shutil
    local_predictions=out/'coco_instances_results.json'; shutil.copy2(predictions,local_predictions)
