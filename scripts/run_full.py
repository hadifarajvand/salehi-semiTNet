...
def main():
    ...
    manifest={'dataset':str(dataset),'num_gpus':a.num_gpus,'burnin_iter':a.burnin_iter,'batch_size_total':a.batch_size,'init_weights':a.init_weights or None}
    out=ROOT/'outputs/full'; out.mkdir(parents=True,exist_ok=True); (out/'run_manifest.json').write_text(json.dumps(manifest,indent=2))
    subprocess.run(['bash','project_train_teacher.sh'],cwd=vendor,env=env,check=True)
    subprocess.run(['bash','project_train_student.sh'],cwd=vendor,env=env,check=True)
